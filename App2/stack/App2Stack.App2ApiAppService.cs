...
namespace App2.stack
{
    ...
    {
        private static WebApp CreateApp2ApiAppService(App2DeploymentConfigs deploymentConfigs, ResourceGroup resourceGroup)
        {
            var appServicePlan = new AppServicePlan($"{deploymentConfigs.ResourcesNames["App2ApiAppServicePlanName"]}", new AppServicePlanArgs
            {
                Name = $"{deploymentConfigs.Client.ToLower()}-{deploymentConfigs.Prefix.ToLower()}-{deploymentConfigs.ResourcesNames["App2ApiAppServicePlanName"]}",
            ...
            var appInsights = new Component($"{deploymentConfigs.ResourcesNames["App2ApiAppInsightsName"]}", new ComponentArgs
            {
                ResourceName = $"{deploymentConfigs.Client.ToLower()}-{deploymentConfigs.Prefix.ToLower()}-{deploymentConfigs.ResourcesNames["App2ApiAppInsightsName"]}",
                ResourceGroupName = resourceGroup.Name,
                Location = resourceGroup.Location,
                Tags = deploymentConfigs.CommonTags,
            ...
            var appService = new WebApp($"{deploymentConfigs.ResourcesNames["App2ApiAppServiceName"]}", new WebAppArgs
            {
                Name = $"{deploymentConfigs.Client.ToLower()}-{deploymentConfigs.Prefix.ToLower()}-{deploymentConfigs.ResourcesNames["App2ApiAppServiceName"]}",
                Location = resourceGroup.Location,
                ResourceGroupName = resourceGroup.Name,
                ...
                {
                    AlwaysOn = true,
                    LinuxFxVersion = deploymentConfigs.App2ApiLinuxFxVersion,
                    AppSettings = GetApp2ApiAppSettings(deploymentConfigs, appInsights),
                    ConnectionStrings = GetApp2ApiConnStrings(deploymentConfigs),
                    HealthCheckPath = deploymentConfigs.App2ApiAppSettings["HealthCheck"].ToString() ?? "/api/health",
                },
                Tags = deploymentConfigs.CommonTags
            });
        ...
        private static NameValuePairArgs[] GetApp2ApiAppSettings(App2DeploymentConfigs deploymentConfigs, Component appInsights)
        {
        ...
        private static void AddApiSettings(List<NameValuePairArgs> settings, App2DeploymentConfigs deploymentConfigs)
        {
            var apiSettings = deploymentConfigs.App2ApiAppSettings;
            ...
            // App2 Events
            if (apiSettings.TryGetValue("App2Events", out var app2EventsObj) && app2EventsObj is Dictionary<string, object> app2EventsDict)
            {
                if (app2EventsDict.TryGetValue("topicEndpoint", out var topicEndpoint))
                    settings.Add(new() { Name = "App2Events__topicEndpoint", Value = topicEndpoint.ToString()! });
            }
        ...
        private static ConnStringInfoArgs[] GetApp2ApiConnStrings(App2DeploymentConfigs deploymentConfigs)
        {
            var secrets = deploymentConfigs.Secrets;
