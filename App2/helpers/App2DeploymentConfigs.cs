...
{
    public record App2DeploymentConfigs
    {
        ...
        public string App2ApiLinuxFxVersion { get; init; }
        public string App2FnLinuxFxVersion { get; init; }
        ...
        public Dictionary<string, object> App2ApiAppSettings { get; init; }
        public Dictionary<string, object> App2FnAppSettings { get; init; }

        // Secure secret access - fixed with flat structure
        public App2SecretAccess Secrets { get; init; }

        public App2DeploymentConfigs(Config config)
        {
            ...
            var apiSettingsRaw = config.RequireObject<JsonElement>("App2ApiAppSettings");
            App2ApiAppSettings = ConfigParser.ConvertJsonElementToDictionary(apiSettingsRaw);

            var fnSettingsRaw = config.RequireObject<JsonElement>("App2FnAppSettings");
            App2FnAppSettings = ConfigParser.ConvertJsonElementToDictionary(fnSettingsRaw);

            // Docker FX versions using shared docker settings
            App2ApiLinuxFxVersion = $"DOCKER|{sharedDockerSettings["DockerRegistryUrl"]}/{DockerSettings["DockerApp2ApiImageName"]}:{DockerSettings["DockerApp2ApiImageTag"]}";
            App2FnLinuxFxVersion = $"DOCKER|{sharedDockerSettings["DockerRegistryUrl"]}/{DockerSettings["DockerApp2FnImageName"]}:{DockerSettings["DockerApp2FnImageTag"]}";

            ConfigNameSeparator = DeploymentKind.ToLower() == "linux" ? "__" : ":";

            // Secret access with shared config
            Secrets = new App2SecretAccess(config, sharedConfig);
        }
    }
}
